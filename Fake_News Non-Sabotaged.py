...
catID = CategoricalDtype(categories = [0,1], ordered = False)
...
pd.DataFrame(matrix, 
             columns=['Predicted ' + str(label) for label in labelNames.astype(str)],
             index=['Is ' + str(label) for label in labelNames.astype(str)])
